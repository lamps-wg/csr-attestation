...
populate it with the formats that they wish to support. This document does not define, allocate, or register an OID for any attestation-statement format, nor does it establish an IANA registry for such formats. A specification or vendor that defines an attestation-statement format is responsible for assigning its OID from an arc under its control.
