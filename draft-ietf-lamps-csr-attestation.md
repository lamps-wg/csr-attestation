...
## The Presenter role for CSRs constructed externally to the Attester {#impl-presenter}
...
cryptographic hardware. More generally, a Presenter could be a TLS library or VPN client
that is presenting attestation data as part of a connection handshake
or other similar "application layer" entities that need to coordinate between an underlying
Attester and the Relying Party.
...
Attestation |    | Attestation
    Request |    | Data
     ...
     '-----------------'                 '-----------------'
~~~
{: #fig-presenter-arch title="Architecture including a Presenter"}


The communication flow between the Presenter and HSM is envisioned to
behave as described in {{fig-csr-client-p11}}, but will of course depend
on the APIs exposed by the underlying cryptographic hardware.
